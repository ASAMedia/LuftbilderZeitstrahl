...
// By default libvips parallelises ONE image across every CPU core, so several
// concurrent pipelines (see procLimit) saturate the whole server on a burst of
// aerial frames. Pin it to a few threads per op and disable the operation
// cache (we process each frame once, then it's on disk). Tunable per host.
...
// Lower this (e.g. 2200) on a CPU-constrained host: WebP encode cost scales
// with pixel count. 3000 keeps it sharp; env-tunable without a code change.
const PREVIEW_MAX_PX = Number(process.env.PREVIEW_MAX_PX) || 3000;
const CACHE_VER = 'v5';
...
// How many frames process in parallel. Total worker threads ≈
// PROC_CONCURRENCY × SHARP_CONCURRENCY, so this stays small by default.
const procLimit = makeSemaphore(Number(process.env.PROC_CONCURRENCY) || 2);
